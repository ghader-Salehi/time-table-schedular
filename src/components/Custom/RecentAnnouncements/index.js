...
const Index = ({ role, content }) => {
  ...
  const handleButtonStyle = (role) => {
    if (role === ADMIN) return classes.AdminButtonStyle;
    else if (role === MASTER) return classes.MasterButtonStyle;
    else if (role === STUDENT) return classes.StudentButtonStyle;
  };
  return (
    <>
      <div className="d-flex justify-content-center mt-1 ">
        ...
        <div className="d-flex flex-column  justify-content-center align-items-center col-2 mt-5">
          {!content.length && (
            <div className={"mb-3 justify-content-center"}>
              <Typography
                ...
                variant="subtitle2"
              >
                اطلاعیه ای وجودندارد
              </Typography>
            </div>
          )}

          <Button
            className={clsx([classes.font, handleButtonStyle(role), "shadow"])}
          >
            {role === STUDENT ? (
              <Typography
                style={{ fontSize: "13px" }}
